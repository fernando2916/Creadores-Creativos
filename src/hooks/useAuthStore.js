...
import { onRegister, clearErrorMessage, onLogin, onLogout, onChecking, onActivate, onActivateFail, onResetCode, onResetCodeFail, onResetPassword, onResetPasswordFail, newPassword, newPasswordFail, onRegisterFail, onLoginFail, } from "@/store";
;
...
export const useAuthStore = () => {
    ...
    const {status, user, errorMessage, message, loading } = useSelector( state => state.auth );
    ...
    const verify = async({id, verification_code}) => {
        ...
        try {
            const {data} = await creadoresApi.post(`/auth/verify/${id}`, {verification_code})
    ...
    const reset_code = async({email}) => {
        ...
        try {
            const {data} = await creadoresApi.post('/auth/reset-verification-code', {email})
    ...
    const startLogin = async({email, password }) => {
            ...
            try {            
                const { data } = await creadoresApi.post('/auth/login', {email, password})
    ...
    const reset_password = async({email}) => {
        ...
        try {
            const {data} = await creadoresApi.post('/auth/reset-password', {email})
    ...
    const new_password = async({password, password_confirmation, id}) => {
        dispatch( onChecking() );
        try {
            const {data} = await creadoresApi.post(`/auth/new-password/${id}`, {password, password_confirmation})
