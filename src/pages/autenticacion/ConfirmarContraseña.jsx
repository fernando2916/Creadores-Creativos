...
const ConfirmarContraseña = () => {
  ...
  const {new_password, errorMessage, message, status, loading} = useAuthStore()
  const id = params.id
  const Navigate = useNavigate()

  const newPassword = (values) => {
    distpatch(new_password({
      id,
  ...
  return (
    <AuthSection title={"Nueva Contraseña"}>
      <>
        ...
        <Formik initialValues={newPasswordChange} onSubmit={newPassword} validationSchema={newPasswordSchema}>
        {({
          values,
          errors,
          handleChange,
        ...
        <form className="pt-3 space-y-3" onSubmit={handleSubmit}>
          <div className="flex flex-col gap-3 relative">
            ...
            <p className="text-sm text-red-400 font-semibold">{errors.password2}</p>
